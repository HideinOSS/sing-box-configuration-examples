...
* 配置样例服务端是由sing-box实现的ShadowTLS V3协议，sing-box作为客户端使用。
* 一般普遍用法是sing-box服务端运行在各Linux发行版上，电脑、手机、平板作为客户端。
* 要使用其他协议参考官方文档修改服务端入站、客户端出站。
* sing-box官方文档地址：https://sing-box.sagernet.org/zh/configuration 。

## 目录解释

* sing-box-GeoIP-Geosite：指sing-box≤1.7.8版本使用GeoIP、GeoSite进行国内外、私有地址分流。sing-box 小于等于1.7.8的版本可以使用，已在sing-box 1.8.0及以后的版本废弃。
* sing-box-RuleSet：指从sing-box 1.8.0版本后使用规则集方式进行国内外、私有地址分流，更加灵活。
* IPv4：服务端和客户端都使用IPv4方式进行配置，受Golang的特性，监听IPv4地址也使用"::"形式，它等同于"0.0.0.0"，IPv4和IPv6服务端都是 ["listen": "::",] ，为了和官方一致不做修改。
...
* 配置文件和官方文档相比，不存在的字段表示为空或使用内置默认值，你可以完全理解官方文档后进行修改。
