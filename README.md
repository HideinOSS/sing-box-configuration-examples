...
* 配置样例服务端是`sing-box`实现的`ShadowTLS v3`协议，各平台`sing-box`作为客户端
* `sing-box`作为服务端或是客户端取决于你的配置文件，它二进制文件不区分服务端客户端
* 后续添加`AnyTLS + Reality`协议，需要`sing-box ≥ 1.12.0`
* 修改配置需要有基本的JSON语法基础
* 使用其他协议参考官方文档修改服务端的入站、客户端的出站，样例默认添加了广告屏蔽规则集，不需要可删除
...
* 文件名包含`mixed`表示使用`Mixed`模式，`mixed入站是一个 socks4, socks4a, socks5 和 http 服务器。`
* 文件名包含`tun`表示使用`Tun`模式，它在目标系统上创建一张虚拟网卡接管流量，在Windows上需要管理员权限

## 重要提示
* [`重要提示`] 为了防止DNS解析泄露，Tun模式启用了防DNS泄漏的规则，防止国内第三者知道你访问了哪些网站。使用`https://ipleak.net` 、`https://browserleaks.com/dns` 来检测现有软件的DNS是否泄露
* [`重要提示`] 如果你不完全理解官方文档，尽量不要自己配置DNS分流，配置不合理会造成DNS泄露
* [`重要提示`] Shadowrocket（AKA “小火箭”）会泄露你的DNS记录，它只在配置文件中让`ipleak`、`browserleaks`等检测网站走代理，让你看起来没有DNS泄漏，这是掩耳盗铃的做法，具体可以查看它的配置文件
...
* 如果发现配置文件在Git提交时自动转换成`CR LF`换行符需要手动改成`LF`换行符
...
* 在Windows 10 LTSC Tun模式中，sing-box ≤ 1.10.0，如果出现DNS解析正常，出站异常，`stack`需设置为：`"stack": "gvisor"`
## 在Windows上使用sing-box
...
### Tun模式
* 其他同上，双击bat后启用管理员权限，不用右键`以管理员身份运行`
 ```bat
  @echo off
  net session >nul 2>&1
  if %errorlevel% neq 0 (
